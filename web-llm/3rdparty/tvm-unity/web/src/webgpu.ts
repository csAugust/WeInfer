const LOG_CreatedBufferNums = false;
const LOG_PassTimeCost = false;
const LOG_CreatedUniformValues = false;
const USE_UNIFORM_CACHE = true;
...
export class WebGPUContext {
  ...
  // FOR LOG
  private uniformBufferCreateCnt = 0;
  private uniformBufferCreateRecord = [];
  private storageBufferCreateCnt = 0;
  private storageBufferCreateRecord = [];
  private stageBufferCreateCnt = 0;
  private resolveResultBuffers = [];

  // FOR CACHE
  private uniformBufferCache = new Map<String, Object>();
  private uniformCacheStatus = { miss: 0, hit: 0 };
    ...
    const createShaderFunc = (pipeline: GPUComputePipeline): Function => {
      const submitShader = (...args: Array<GPUPointer | number>): void => {
        ...
        if (LOG_PassTimeCost) {
          ...
          compute = commandEncoder.beginComputePass({ timestampWrites });
        } else {
        ...
        if (USE_UNIFORM_CACHE && this.uniformBufferCache.has(uniformValuesKey)) {
          ...
          if (LOG_CreatedUniformValues) {
            // console.log(`@Create Uniform: #${podArgIndices.length + 1}`, i32View);
            this.uniformBufferCreateRecord.push({
              allocSize: (podArgIndices.length + 1) * sizeOfI32,
              i32Cnt: podArgIndices.length + 1,
              i32Vals: i32View,
            });
          }
        }
        ...
        if (LOG_PassTimeCost) {
          ...
          this.resolveResultBuffers.push({
            fname: finfo.name,
            resultBuffer,
            state: "waiting",
          })
  ...
  gpuBufferFromPtr(ptr: GPUPointer): GPUBuffer {
  ...
  private logBufferStatus(): void {
    if (!LOG_CreatedBufferNums)
      return;
    console.log("uniform | storage | stage")
    console.log(this.uniformBufferCreateCnt, this.storageBufferCreateCnt, this.stageBufferCreateCnt);
  }

  private logPassCostStatus(): void {
    if (!LOG_PassTimeCost)
      return;
    const resPromises = this.resolveResultBuffers.map(res => {
      if (res.state === 'waiting') {
        res.state = 'pending';
        return res.resultBuffer.mapAsync(GPUMapMode.READ);
      } else {
        return Promise.resolve(undefined);
      }
    })
    Promise.all(resPromises).then(() => {
      this.resolveResultBuffers.forEach((res) => {
        // @Log Per function call pass's cost
        if (res.state === 'pending') {
          const times = new BigInt64Array(res.resultBuffer.getMappedRange());
          console.log(res.fname);
          console.log(`Compute pass duration: ${Number(times[1] - times[0])}ns`);
          res.resultBuffer.unmap();
          res.resultBuffer.destroy();
          res.state = 'furfilled';
        }
      })
      this.resolveResultBuffers = this.resolveResultBuffers.filter((res) => { return res.state === 'waiting' || res.state === 'pending'; })
    });
  }

  private logUniformStatus(): void {
    console.log(this.uniformCacheStatus)
    if (LOG_CreatedUniformValues) {
      console.log(this.uniformBufferCreateRecord)
    }
  }
}
